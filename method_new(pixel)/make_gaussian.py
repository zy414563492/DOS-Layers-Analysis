    ...
    for i in range(128):
        for j in range(128):
            if label_matrix[i, j] != 0:
                data_center[i][j] = [center_mean, center_mean, center_mean]  # [0.5, 0.5, 0.5]  # [0, 0, 0]  # [np.nan, np.nan, np.nan] # 四周平均
